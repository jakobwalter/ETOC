...
GetModels <- function(t1Data, formula, sp = c(0.25, 0.25, 0.25)){
  paramGrid <- expand.grid(Model = unique(t1_data$name),
                            n =  as.character(unique(t1_data$n)),
                            DE = c(T,F))
  fittedModels <- apply(paramGrid, 1, function(p){
    mask <- t1Data$n == p["n"] & t1Data$de_mask == p["DE"] & t1Data$name == p["Model"]
...
GetEmpAlpha <- function(data, alphaTarget){
  res <- list()
  ### First: Unfiltered Genes for n == 10
  ...
  ### First: Unfiltered Genes for n == 40
