...
suppressMessages(suppressWarnings(
  ...
  replicate(nRepl,{
    ...
    res$DP <- DirectPermutationsPipeline(XYData$X, XYData$Y, nFlips = 400)$p.value
    res$FsP.B     = FlipScoresPipeline(XYData$X, XYData$Y, "poisson",
                                       scoreType = "basic", nFlips = 200)$p.value
    res$FsP.E = FlipScoresPipeline(XYData$X, XYData$Y, "poisson",
                                   scoreType = "effective", nFlips = 200)$p.value
    res$FsP.S  = FlipScoresPipeline(XYData$X, XYData$Y, "poisson",
...
suppressMessages(suppressWarnings(
  ...
  replicate(n_repl,{
    ...
    res$MW       <- MWUPipeline(XY_data$X, XY_data$Y)$p.value
    res$DP       <- DPPipeline(XY_data$X, XY_data$Y, n_flips = 200)$p.value
    res$FsPoisB  <- FlipScoresPipeline(XY_data$X, XY_data$Y, "poisson",
                                       score_type = "basic", n_flips = 40)$p.value
    res$FsPoisE  <- FlipScoresPipeline(XY_data$X, XY_data$Y, "poisson",
                                   score_type = "effective", n_flips = 40)$p.value
    res$FsPoisS  <- FlipScoresPipeline(XY_data$X, XY_data$Y, "poisson",
