...
rownames(tcga_data$counts) <- 1:nrow(tcga_data$counts)
```
...
#get random genes 
DE  <- sample(which(DE), DE_size)
NDE <- sample(which(NDE), DE_size*5)

X_DE  <- tcga_data$counts[DE,]
X_NDE <- tcga_data$counts[NDE,]

X_all <- rbind(X_DE, X_NDE)
rownames(X_all) <- 1:nrow(X_all)

Y_all <- tcga_data$group == "Primary solid Tumor"

#compute norm factors for permutations
d_all <- edgeR::DGEList(counts = X_all, group = Y_all)
d_all <- edgeR::calcNormFactors(d_all)
d_all <- edgeR::estimateDisp(d_all)

#rm(tcga_data, LIHC_res, LIHC_res_DE, LIHC_res_NDE)

c_all <- abs(cor(t(edgeR::cpm(d_all))))
diag(c_all) <- 0
c_max <- apply(c_all, 2, max)

plot_data <- data.frame(mu = rowMeans(d_all$counts * d_all$samples$norm.factors),
                        phi = d_all$tagwise.dispersion,
                        corr_max = c_max,
                        de_mask = 1:nrow(d_all) <= DE_size)

rm(c_max, c_all)

n_repl <- 200
ResFileName <- paste0(data.dir, "LIHCPermutationResults.Rds")
...
PermResults$plot_data$Unfiltered = plot_data
...
suppressMessages(suppressWarnings(
  ...
  PermResults$GeneData$Unfiltered <- lapply(n_list, function(n){
  print(n)
  c <- 0
  t_n <- Sys.time()
  ...
  replicate(n_repl,{
    cat("\r",
        "Progress: ", round(c/n_repl,3),  
        "Time Remaining: ", 
        round(difftime(Sys.time(), t_n, units = "mins")/(c/n_repl)*(1- c/n_repl), 2), " min    ")
    c <<- c + 1
    XY_data <- GetXYperm(n, X_DE = X_DE, X_NDE = X_NDE, Y_all, DE_size,
                         d_all$samples$norm.factors)
    res <- list()
    res$EdgerEt  <- EdgeRPipeline(XY_data$X, XY_data$Y, test = "exact",
                                  filtering = F)$p.value
    res$EdgerQl  <- EdgeRPipeline(XY_data$X, XY_data$Y, test = "ql",
                                     filtering = F)$p.value
    res$DESeq2   <- DESeq2Pipeline(XY_data$X, XY_data$Y)$p.value
    res$Limma    <- LimmaPipeline(XY_data$X, XY_data$Y, filtering = F)$p.value
    res$MW       <- WilcoxPipeline(XY_data$X, XY_data$Y)$p.value
    res$DP <- DirectPermutationsPipeline(XY_data$X, XY_data$Y, n_flips = 2000)$p.value
    res$FsP.B     = FlipScoresPipeline(XY_data$X, XY_data$Y, "poisson",
                                       score_type = "basic", n_flips = 400)$p.value
    res$FsP.E = FlipScoresPipeline(XY_data$X, XY_data$Y, "poisson",
                                   score_type = "effective", n_flips = 400)$p.value
    res$FsP.S  = FlipScoresPipeline(XY_data$X, XY_data$Y, "poisson",
                                    score_type = "standardized", n_flips = 400)$p.value
    return(res)
    })
  })
))
...
DE <- rowSums(DE, na.rm = T) >= 6
NDE <- rowSums(NDE, na.rm = T) >= 4
...
PermResults$plot_data_Filtered <- plot_data
saveRDS(PermResults, file = ResFileName)
```
...
suppressMessages(suppressWarnings(
  ### For Each Sample Size n:
  PermResults$GeneData$filtered <- lapply(n_list, function(n){
  print(n)
  c <- 0
  t_n <- Sys.time()
  ### Inner Loop: Replicate n_repl times
  replicate(n_repl,{
    ...
    rn <- rownames(XY_data$X)
    ### Filtering
    d <- edgeR::DGEList(counts = XY_data$X)
    ...
    res$FsP.B     = FlipScoresPipeline(XY_data$X, XY_data$Y, "poisson",
                                       score_type = "basic", n_flips = 40)$p.value
    res$FsP.E = FlipScoresPipeline(XY_data$X, XY_data$Y, "poisson",
                                   score_type = "effective", n_flips = 40)$p.value
    res$FsP.S  = FlipScoresPipeline(XY_data$X, XY_data$Y, "poisson",
                                    score_type = "standardized", n_flips = 40)$p.value
...
```
```{r}
PermResults$GeneData$filtered$"10"[1,]
```


```{r}
rm(d_all, LIHC_res, LIHC_res_DE, n_list, plot_data, tcga_data, X_all, X_DE,
   X_NDE, mask_na)
```
