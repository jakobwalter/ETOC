...
phi <- d$tagwise.dispersion
mu <- rowMeans(d$counts*d$samples$norm.factors)
mu_1 <- mu_2 <-  mu[mu > 10][1:pGenes]
phi_1 <- phi_2 <- phi[mu > 10][1:pGenes]
...
RunAllMethodsCovariates <- function(XYData){
  suppressMessages(suppressWarnings({
    ...
    res$EdgeREt  <- edgeRPipelineCovariates(XYData)
    res$DESeq2   <- DESeq2PipelineCovariates(XYData)
    res$FsPB     <- FlipScoresPipelineCovariates(XYData, "poisson", scoreType = "basic", nFlips = 200)
    res$FsPE     <- FlipScoresPipelineCovariates(XYData, "poisson", scoreType = "effective", nFlips = 200)
    res$FsPS     <- FlipScoresPipelineCovariates(XYData, "poisson", scoreType = "standardized", nFlips = 200)
    res$FsNBB     <- FlipScoresPipelineCovariates(XYData, "negbinom", scoreType = "basic", nFlips = 200)
    res$FsNBE     <- FlipScoresPipelineCovariates(XYData, "negbinom", scoreType = "effective", nFlips = 200)
    res$FsNBS     <- FlipScoresPipelineCovariates(XYData, "negbinom", scoreType = "standardized", nFlips = 200)
  }))
  return(res)
}
```
...
genResults$geneData <- lapply(nList, function(n){
  ...
  replicate(nRepl,{
    ...
    XYData <- SimulateNB(n, pGenes, mu1 = mu_1, mu2 = mu_2, phi1 = phi_1, phi2 = phi_1, beta = rnorm(pGenes, 0, 1))
