...
cowplot::save_plot(p_all, filename = fileName, nrow =2,
                   base_height = 3.05)
```
...
fdr <- lapply(permResults$geneData$unfiltered, function(nDf){
  fdrDf <- sapply(1:9, function(i){
    ...
    df <- apply(df, 2, p.adjust, method = "BH") <= 0.1
...
fdr <- lapply(permResults$geneData$unfiltered, function(nDf){
  fdrDf <- sapply(1:9, function(i){
    df <- as.data.frame(nDf[i,])
    df <- apply(df, 2, p.adjust, method = "BH") <= 0.1
    fdr <- 1 - colSums(df[permResults$plotData$filtered$deMask,], na.rm = T) / colSums(df, na.rm = T)
    names(fdr) <- NULL
    fdr
  })
  fdrDf <- replace_na(fdrDf, 0)
...
ggsave(file_name, p1, width = 4, height = 5)
...
fileName <- paste("../plots/Perm", gsub(" ", "_", title_name, fixed = T), "AdjustedUnfiltered.png", sep = "")
...
p_all <- GetLinePlots(t1Preds$nominal$unfiltered, labelOrder[1:4], DE = "NDE", title =  title_name, ymax = 0.5)
...
p_all <- GetLinePlots(t1Preds$nominal$unfiltered, labelOrder[5:9], DE = "NDE", title =  title_name, ymax = 0.5)
file_name_H0 <- paste("../plots/Perm", gsub(" ", "_", title_name, fixed = T), ".png", sep = "")
cowplot::save_plot(p_all, filename = file_name_H0, nrow =3)
