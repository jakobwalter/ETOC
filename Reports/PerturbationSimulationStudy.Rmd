...
mask <- rowSums(is.na(df))
hist(mask, breaks = 100)
mask <- mask < 200
df <- df[!permResults$plotData$unfiltered$deMask & mask,]
...
```{r}
permResults$geneData$unfiltered$"12"
```
...
fdr <- lapply(permResults$geneData$unfiltered, function(nDf){
  ...
  colnames(fdrDf) <- row.names(permResults$geneData$unfiltered$"12")
  fdrDf
})
