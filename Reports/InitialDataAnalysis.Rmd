...
if (file.exists(paste0(dataDir, "TCGALIHC.Rds")) | reproList$downloadingRawData){
  ...
  TCGADataRaw <- readRDS(file = paste0(dataDir, "TCGALIHC.RDS"))
  ...
  TCGADataRaw <- TCGAbiolinks::GDCprepare(queryTCGA,
                                          directory = dataDir)

  TCGADataRaw <- list(counts = assay(TCGADataRaw),
                        patient = TCGADataRaw$patient,
                        group = TCGADataRaw$definition)
  ### Filter relevant data and Save Data to Specified path
  saveRDS(object = TCGADataRaw,
...
duplicatedMask <- duplicated(TCGADataRaw$patient)
sum(duplicatedMask)
patientMask <- TCGADataRaw$patient %in% TCGADataRaw$patient[duplicatedMask]

### The duplicate samples are coming from paired data, for which we delete
### the samples corresponding to the tumor.
patientMaskDelete <- patientMask & TCGADataRaw$group == "Primary Tumor"

### Let us now compute the normalization factors and plot the MDS of the data

#### Compute EdgeR object
dge   <- DGEList(counts = TCGADataRaw$counts[,!patientMaskDelete],
                 group  = TCGADataRaw$group[!patientMaskDelete])
...
TCGADataRaw$patientMaskDelete <- patientMaskDelete| TCGADataRaw$sample_type == "Recurrent Tumor"


saveRDS(object = TCGADataRaw,
          file = paste0(dataDir, "TCGALIHC.Rds"))

rm(dge, MDSData, MDSPlot, TCGADataRaw, duplicatedMask, keep, patientMask, 
   patientMaskDelete)
```
...
### Load Data
tcgaData <- readRDS(paste0(dataDir, "TCGALIHCProcessed.Rds"))
if (file.exists(paste0(dataDir, "TCGALIHCResults.Rds"))){
  LIHCRes <- readRDS(paste0(dataDir, "TCGALIHCResults.Rds"))
} else {
  LIHCRes <- list()
}
```
...
design <-  model.matrix(~ tcgaData$group)
dge    <- edgeR::DGEList(counts=tcgaData$counts)
...
LIHCRes$Limma <- df
saveRDS(LIHCRes, paste0(dataDir, "TCGALIHCResults.Rds"))
...
### Save Data
LIHCRes$EdgeREt <- et$table %>% dplyr::select(PValue)
saveRDS(LIHCRes, paste0(dataDir, "TCGALIHCResults.Rds"))
...
pVals <- qlf$table %>% dplyr::select(PValue)

### Save Data
LIHCRes$EdgeRQl <- pVals
saveRDS(LIHCRes, paste0(dataDir, "TCGALIHCResults.Rds"))
```

```{r DESeq2}
### Initialize Object
Y   <- data.frame("treatment" = as.factor(tcgaData$group))
dds <- DESeq2::DESeqDataSetFromMatrix(tcgaData$counts,
                                      colData = Y,
                                      design = ~ treatment)
...
### Save Data
LIHCRes$DESeq2 <- topGenes %>% dplyr::select(pvalue)
saveRDS(LIHCRes, paste0(dataDir, "TCGALIHCResults.Rds"))
```
...
```{r MW Test}
### Normalize Data
groupMask <-  tcgaData$group == tcgaData$group[1] #Makes Data Binary
dge <- edgeR::DGEList(counts= tcgaData$counts)
...
### Apply Wilcoxon Test to All Genes using the created mask and Save Data
LIHCRes$MW <- sapply(1:nrow(dgeCpm), function(i) {
...
nFlips <- 2000
rn <- row.names(tcgaData$counts)

### Normalize Data
d  <- edgeR::DGEList(counts = tcgaData$counts)
d  <- edgeR::calcNormFactors(d)
os <- edgeR::getOffset(d)
X  <- d$counts * d$samples$norm.factors
Y  <- tcgaData$group == tcgaData$group[1]

t0 <- Sys.time()
pVals <- sapply(1:nrow(tcgaData$counts), function(i){
...
### Prepare Data
Y <- (tcgaData$group == tcgaData$group[1])*2-1 # Data being encoded (-1, 1)


### Loop through Columns and apply Flipscores test to each
t0 <- Sys.time()
pVals <- sapply(1:nrow(tcgaData$counts), 
                 function(i) {
                   ### print progress
                   if(i %% 50 == 0){PrintProgress(i, nrow(tcgaData$counts), t0)}
                   X_i <- d$counts[i,] 
                   suppressWarnings({
...
rm(d, fileName, os, pVals, rn, t0, Y)
```

# Visualizing Results
```{r Setup Visualization}
namesList <- names(LIHCRes)
LIHCRes <- as.data.frame(LIHCRes)
