...
```{r, eval = reproList$downloadingRawData}
if (reproList$downloadingRawData){
  ### Download Data
...
(MDSPlot <- ggplot(data = as.data.frame(list(x = MDSData$x, y = MDSData$y,
  ...
  +
  scale_color_discrete(name = "Condition", labels = c("Primary Tumor", 
                                                      "Recurrent Tumor",
                                                      "Normal Tissue")
  ) +
...
rm(dge, MDSData, MDSPlot, TCGAData, duplicatedMask, keep, patientMask, patientMaskDelete)
```

We removed `nRemoved` samples to make our samples independent.

```{r}
table(tcgaDataCleaned$sampleType)
length(tcgaDataCleaned$sampleType)
```
...
```{r EdgeR QLM Test, eval = reproList$computingTCGALIHCResults}
design <- model.matrix(~ tcgaDataCleaned$group)
...
```{r DESeq2, eval = reproList$computingTCGALIHCResults}
### Initialize Object
...
```{r MW Test, eval = reproList$computingTCGALIHCResults}
### Normalize Data
...
```{r Direct Permutation Test, eval = reproList$computingTCGALIHCResults}
nFlips <- 2000
...
```{r Flipscores, eval = reproList$computingTCGALIHCResults}
...
as.matrix(corSpear)
mds <- MASS::isoMDS(1- corSpear +0.00001)
p1 <- ggplot(mapping = aes(x = mds$points[,1], y = mds$points[,2],
                     col = row.names(mds$points))) +
  geom_point() +
  labs(x = "x", y = "y", title = "MDS of 1 - Spearman correlation between all methods") +
